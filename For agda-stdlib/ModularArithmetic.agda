...
modsuc-helper-eq-invariant : 
    (m k j : ℕ) → (eq eq′ : j ℕ.+ k ≡ m) → (n : ℕ) 
    → modsuc-helper m k j eq n ≡ modsuc-helper m k j eq′ n
modsuc-helper-eq-invariant m k j refl refl n = refl 

modsuc-helper-j-invariant′ : 
    (m k j j′ : ℕ) 
    → (eq : j ℕ.+ k ≡ m) → (eq′ : j′ ℕ.+ k ≡ m) → (eqj : j ≡ j′) 
    → (n : ℕ) 
    → modsuc-helper m k j eq n ≡ modsuc-helper m k j′ eq′ n
modsuc-helper-j-invariant′ .(j ℕ.+ k) k j .j refl refl refl n = refl

modsuc-helper-j-invariant : 
    (m k j j′ : ℕ) → (eq : j ℕ.+ k ≡ m) → (eq′ : j′ ℕ.+ k ≡ m) → (n : ℕ) 
    → modsuc-helper m k j eq n ≡ modsuc-helper m k j′ eq′ n
...
modsuc-zero m = (cong-app {_} {_} {ℕ} {λ _ → Fin (suc m)} {modsuc m} {modsuc-helper m 0 m (+-identityʳ m)} refl zero)
...
modsuc-helper-steps : 
    (m k j : ℕ) → (eq : j ℕ.+ k ≡ m) → (n : ℕ) 
    → modsuc-helper m k j eq n ≡ modsuc-helper m zero m (+-identityʳ m) (k ℕ.+ n)
modsuc-helper-steps .(j ℕ.+ zero) zero j refl n = 
    modsuc-helper-j-invariant (j ℕ.+ zero) zero j (j ℕ.+ zero) refl (+-identityʳ (j ℕ.+ zero)) n
modsuc-helper-steps .(j ℕ.+ suc k) (suc k) j refl n = 
    trans 
        (trans 
            (modsuc-helper-eq-invariant (j ℕ.+ suc k) (suc k) j refl (trans (+-suc j k) (sym (+-suc j k))) n) 
            (modsuc-helper-steps (j ℕ.+ suc k) k (suc j) (sym (+-suc j k)) (suc n))
        ) 
        (cong 
            (λ x → modsuc-helper 
                (j ℕ.+ suc k) 
...
modsuc≤m {m} {a} (s≤s lt) = 
    trans 
        (trans 
            (cong 
                (λ x → modsuc-helper m zero m (+-identityʳ m) x) 
                (sym (+-identityʳ a))) 
            (sym 
                (modsuc-helper-steps m a (≤toDiff (s≤s lt)) (≤toDiffProof lt) 0)
            )
        ) 
        (fromℕ<-lt-invariant 
            (diff (≤toDiff lt) (trans (+-suc (≤toDiff lt) a) (cong suc (≤toDiffProof lt)))) 
            (s≤s lt)
        )
...
modsuc∘suc : (m a : ℕ) → modsuc m (suc a) ≡ sucFin (modsuc m a)
modsuc∘suc m a = trans {!   !} (erefl (passdown (suc m) suc (modsuc m a))) 

-- Algebraic Properties

passdownPreservesCommutativity : {m : ℕ} → (_·_ : Op₂ ℕ) → Commutative _≡_ _·_ → Commutative _≡_ (passdown m _·_) 
passdownPreservesCommutativity {suc m} _·_ comm-pf a b = 
    trans 
        (cong (λ f → f a b) {passdown (suc m) _·_} {λ a b → modsuc m (F.toℕ a · F.toℕ b) } refl) 
        (trans 
            (cong (modsuc m) (comm-pf (toℕ a) (toℕ b))) 
            (sym ((cong (λ f → f b a) {passdown (suc m) _·_} {λ p q → modsuc m (F.toℕ p · F.toℕ q) } refl)))
        ) 
...
+F-IdR : {m : ℕ} → RightIdentity _≡_ zero (addFin {suc m}) 
+F-IdR {m} a = trans (cong (λ f → f a zero) {addFin {suc m}} {passdown (suc m) ℕ._+_} refl) (trans (cong (λ x → modsuc m x) (+-identityʳ (toℕ a))) (modsuc∘toℕ a))

+F-sucL : {m : ℕ} → (a b : Fin m) → (sucFin a) +F b ≡ sucFin (a +F b)
+F-sucL {suc m} a b = 
    trans 
        (erefl (passdown {1} (suc m) ℕ._+_ (passdown (suc m) suc a) b)) 
        (trans 
            (trans {!   !} {!   !}) 
            (erefl (passdown (suc m) suc (passdown {1} (suc m) ℕ._+_ a b))))

{-
+F-sucL {suc m} a b = trans (erefl (passdown {1} (suc m) ℕ._+_ (passdown (suc m) suc a) b)) {!   !}
...
modsucIsGroupHomomorphism+ m zero b = 
    trans 
        (sym (+F-IdL (modsuc m b))) 
        (cong (λ x → addFin x (modsuc m b)) {zero} {modsuc m zero} (sym (modsuc-zero m)))
...
+F-assoc {suc m} a b c = 
    trans 
        (cong (λ f → f (f a b) c) {addFin {suc m}} {passdown {1} (suc m) ℕ._+_}  refl) 
        (trans {!   !} (cong (λ f → f a (f b c)) {passdown {1} (suc m) ℕ._+_} {addFin {suc m}} refl))  
